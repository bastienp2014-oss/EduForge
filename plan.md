...
- [ ] **App Check partout** : `requireAppCheck` sur tous les `/api/gemini/*` et `/api/admin/*` ; front migré sur `secureFetch`. *AC : requête sans token App Check → 401 (après 1 semaine en mode monitor).*
- [ ] **Clés IA côté serveur** : BYOK stocké chiffré dans `tenants/{id}/secrets` ; suppression du header `x-api-key` et du fallback silencieux ; quota Gemini par tenant. *AC : clé absente du localStorage ; dépassement quota → 429.*
