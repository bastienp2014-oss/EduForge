...
- [x] **Endpoints debug** : `/api/debug-sentry` reste gardé par `NODE_ENV`, `/api/debug/error` reste en place pour logging. *AC : requête `/api/debug/error` non authentifiée → 403 serveur (actuellement `requireAuth` qui n'est que du middleware, pas du vrai 403).*

Vérifié le 08/07/2026 par inspection directe du code mergé (PR #5) : `/api/debug/error` et `/api/debug-sentry` protégés par `requireAuth, requireSuperAdmin` sans bypass NODE_ENV ni token. Middlewares d'authentification extraits proprement dans `src/middlewares/auth.middleware.ts`. Tests et lint passent.
...
- [x] **Garde-fous RAG v0** : limite de taille d'ingestion + quota tenant + restriction aux rôles admin/creator du tenant. *AC : ingestion 10 Mo → 413.*

Vérifié le 08/07/2026 par inspection directe du code mergé (PR #7) : middleware JSON conditionnel implémenté (10 MB limit pour `/api/gemini/rag-ingest`, 100 KB par défaut pour autres routes). `ragIngestLimiter` (20 ingestions/heure/tenant) en place. Vérification de rôle (`admin`/`creator`) fonctionnelle avec commentaire explicite sur la dette du RAG v0 (index non isolé par tenant, hors scope). Tests Vitest (4 scénarios) passent.
